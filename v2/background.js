...
const discard = tab => {
  ...
  const simultaneousJobs = prefs['simultaneous-jobs'];
  const faviconDelay = prefs['favicon-delay'];
  const updateFavicon = prefs.favicon;
  const prependTitle = prefs.prepends;

  if (discard.count > simultaneousJobs && discard.time + 5000 < Date.now()) {
    discard.count = 0;
  }
  if (discard.count > simultaneousJobs) {
    ...
    const icon = () => {
      ...
      if (cached && Date.now() - cached.time < ICON_TTL) {
        chrome.tabs.executeScript(tab.id, {
          ...
          `
        }, () => setTimeout(next, faviconDelay) && chrome.runtime.lastError);
        return;
      }
...
/* FAQs & Feedback */
{
  ...
  if (navigator.webdriver !== true) {
    ...
    const page = manifest.homepage_url;
